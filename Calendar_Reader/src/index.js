...
var startSearchHandlers = Alexa.CreateStateHandler(states.SEARCHMODE, {
    ...
    'searchIntent': function () {
        ...
        if (slotValue != undefined)
        {
            ...
            ical.fromURL(URL, {}, function (err, date) {
                ...
                for (var k in data) {
                    if (data.hasOwnProperty(k)) {
                        ...
                        // add the newly created object to an array for use late
                ...
                if (eventList.length > 0) {
                    ...
                    if (eventDate.startDate && eventDate.endDate) {
                        ...
                        relevantEvents = getEventsBetweenDates(eventData.startDate, eventDate.endDate, eventList);
...
function getDateFromSlot(rawDate) {
    ...
    // f could not parse data must be one of the other formats
    if (isNaN(date)) {
        // to find out what type of date this is, we can split it and count how many parts we have
        ...
        if (res.length === 2 && res[1].indexOf('W') > -1) {
            ...
            eventDate["startDate"] = new Date(dates.startDate);
            eventDate["endDate"] = new Date(dates.endDate);
            // anything else would be out of range for the skill
        } else {
...
// Loops through the events from the iCal data, and checks which ones are between our start date and our end date
